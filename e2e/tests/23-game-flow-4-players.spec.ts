...
test.describe('Game Flow - 4 Real Players', () => {
  ...
  test.describe('@quick Quick Games', () => {
    test('should complete a game from 35-35 scores', async ({ browser }) => {
      test.setTimeout(120000); // 2 minutes

      // Create game with 4 real players
      const result = await createGameWith4Players(browser);
      pages = result.pages;
      contexts = result.contexts;
      gameId = result.gameId!;

      console.log(`Quick game created with ID: ${gameId}`);

      // Set scores to 35-35 for quick end-game
      ...
      while (!gameEnded && roundsPlayed < maxRounds) {
        roundsPlayed++;
        console.log(`Playing round ${roundsPlayed}`);

        // Simple betting - everyone bets 8
        const bettingOrder = [2, 3, 0, 1]; // P3, P4, P1, P2 (dealer is P2)
      ...
      console.log(`Quick game completed in ${roundsPlayed} rounds`);
    });

    test('should handle close game finish (40-39)', async ({ browser }) => {
      test.setTimeout(120000); // 2 minutes
      ...
      // Set scores to 40-39 for very close game
      await pages[0].evaluate(() => {
        // @ts-ignore
        if (window.socket) {
          // @ts-ignore
      ...
      for (const playerIndex of bettingOrder) {
        const betBtn = pages[playerIndex].getByTestId('bet-7-with-trump');
        const skipBtn = pages[playerIndex].getByTestId('skip-bet-button');
      ...
      // Verify game ended
      await expect(pages[0].locator('text=/Game Over/i')).toBeVisible({ timeout: 10000 });
    });
  });
