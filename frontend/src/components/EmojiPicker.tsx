...
export function EmojiPicker({ onSelectEmoji, onClose }: EmojiPickerProps) {
  ...
  return (
    <div className="absolute bottom-full right-0 mb-2 w-80 bg-white dark:bg-gray-800 border-2 border-blue-500 dark:border-blue-600 rounded-lg shadow-2xl z-50 animate-slide-in">
      {/* Header */}
      <div className={`bg-gradient-to-r ${designTokens.gradients.primary} dark:from-blue-600 dark:to-blue-700 px-3 py-2 rounded-t-md flex items-center justify-between`}>
        <h4 className="text-white font-bold text-sm">Pick an Emoji</h4>
        <button
          onClick={onClose}
          className="text-white hover:text-red-300 transition-colors font-bold focus:outline-none focus:ring-2 focus:ring-white/50"
      ...
      <div className="flex gap-1 p-2 border-b border-gray-300 dark:border-gray-600 overflow-x-auto">
        {Object.keys(EMOJI_CATEGORIES).map((category) => (
          <button
            ...
            className={`px-3 py-1 rounded text-xs font-semibold transition-all whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-blue-400 ${
      ...
      <div className="p-3 grid grid-cols-8 gap-2 max-h-48 overflow-y-auto bg-gray-50 dark:bg-gray-900">
        {EMOJI_CATEGORIES[activeCategory].map((emoji, index) => (
          <button
            ...
            className="text-2xl hover:scale-125 transition-transform hover:bg-blue-100 dark:hover:bg-blue-900 rounded p-1 focus:outline-none focus:ring-2 focus:ring-blue-400"
