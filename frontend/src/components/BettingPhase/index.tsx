 ...
 * - BetAmountSelector: Amount grid (7-12)
 * - TrumpSelector: With/Without trump toggle
 * - BettingActions: Skip/Bet buttons
 * - useBettingKeyboardNav: Keyboard navigation hook
 */
...
import { useBettingKeyboardNav } from './useBettingKeyboardNav';
...
function BettingPhaseComponent({
  ...
  const canSkip = useCallback((): boolean => {
    ...
    return hasValidBets;
  }, [iAmDealer, currentBets]);

  const isBetAmountValid = useCallback(
    (amount: number): boolean => {
      if (!highestBet) return true;
      if (iAmDealer) return amount >= highestBet.amount;
      return amount > highestBet.amount;
    },
    [highestBet, iAmDealer]
  );

  const isCurrentBetValid = useCallback((): boolean => {
    if (!highestBet) return true;
  ...
  }, [highestBet, selectedAmount, withoutTrump, iAmDealer]);

  const handlePlaceBet = useCallback(() => {
    sounds.betPlaced();
    onPlaceBet(selectedAmount, withoutTrump, false);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [selectedAmount, withoutTrump, onPlaceBet]);
  ...
  const { navLevel, actionIndex } = useBettingKeyboardNav({
    isMyTurn,
    hasPlacedBet,
    selectedAmount,
    setSelectedAmount,
              ...
              {isMyTurn ? (
                <>
                  <BetAmountSelector
                    ...
                    isNavActive={navLevel === 0}
                  />

                  <TrumpSelector
                    withoutTrump={withoutTrump}
                    onToggle={setWithoutTrump}
                    isNavActive={navLevel === 1}
                  />

                  <BettingActions
                    canSkip={canSkip()}
                    isCurrentBetValid={isCurrentBetValid()}
                    selectedAmount={selectedAmount}
                    withoutTrump={withoutTrump}
                    onSkip={handleSkip}
                    onPlaceBet={handlePlaceBet}
                    isNavActive={navLevel === 2}
