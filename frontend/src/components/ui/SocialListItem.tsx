...
export function SocialListItem({
  ...
  return (
    <div
      className={`
        bg-skin-secondary rounded-lg p-4
    ...
    >
      ...
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-skin-primary font-semibold truncate">{playerName}</p>
          {metadata && (
            <p className="text-sm text-skin-muted truncate">{metadata}</p>
          )}
          {status && !avatar && (
            <p
              className={`text-sm ${status === 'offline' ? 'text-skin-muted' : 'text-skin-success'}`}
            >
