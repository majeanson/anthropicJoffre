...
export function UISlider({
  ...
  return (
    <input
      ...
      data-testid={testId}
      className={`
        w-full bg-skin-secondary rounded-lg appearance-none cursor-pointer
        ${sizeConfig[size]}
        ${colorConfig[color]}
        ${disabled ? 'opacity-50 cursor-not-allowed' : ''}
        focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 focus:ring-offset-skin-primary
        ${className}
      `}
    />
  );
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-1">
        <span className="text-sm text-skin-secondary font-medium">{fieldLabel}</span>
        {showValueLabel && valueLabelPosition === 'right' && (
          <span className="text-xs text-skin-muted tabular-nums">
            {valueDisplay}
          </span>
        )}
      </div>
      ...
      {showValueLabel && valueLabelPosition === 'below' && (
        <div className="text-center mt-1">
          <span className="text-sm text-skin-muted tabular-nums">
            {valueDisplay}
          </span>
        </div>
      )}
