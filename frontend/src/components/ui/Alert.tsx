...
const defaultIcons: Record<AlertVariant, ReactNode> = {
  ...
  arcane: <span className="text-lg">⚗</span>,
  neutral: (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
      <path
        fillRule="evenodd"
...
export function Alert({
  ...
  return (
    ...
    >
      {/* Corner accents */}
      <div
        className={`absolute top-1 left-1 w-3 h-3 border-l border-t opacity-40 ${style.border}`}
      />
      <div
        className={`absolute top-1 right-1 w-3 h-3 border-r border-t opacity-40 ${style.border}`}
      />
      <div
        className={`absolute bottom-1 left-1 w-3 h-3 border-l border-b opacity-40 ${style.border}`}
      />
      <div
        className={`absolute bottom-1 right-1 w-3 h-3 border-r border-b opacity-40 ${style.border}`}
      />

      {/* Icon */}
      <div className={`flex-shrink-0 text-xl ${style.iconColor}`} aria-hidden="true">
        {icon || defaultIcon}
      </div>
...
/** Info alert */
export const InfoAlert = (props: PresetAlertProps) => <Alert variant="info" {...props} />;

/** Success alert */
export const SuccessAlert = (props: PresetAlertProps) => <Alert variant="success" {...props} />;

/** Warning alert */
export const WarningAlert = (props: PresetAlertProps) => <Alert variant="warning" {...props} />;

/** Error alert */
export const ErrorAlert = (props: PresetAlertProps) => <Alert variant="error" {...props} />;

/** Arcane alert */
export const ArcaneAlert = (props: PresetAlertProps) => <Alert variant="arcane" {...props} />;
