...
export function calculateRoundScoring(game: GameState): RoundScoringResult {
  ...
  // Look up betting player - try by ID first, then by bet matching (in case player was replaced by bot or took over)
  ...
  if (!offensivePlayer) {
    console.error('Failed to find betting player. Game state:', {
      ...
      highestBetAmount: game.highestBet?.amount,
      highestBetWithoutTrump: game.highestBet?.withoutTrump,
      players: game.players.map(p => ({ id: p.id, name: p.name, isBot: p.isBot })),
      currentBets: game.currentBets
    });
    throw new Error('Betting player not found - unable to determine who made the highest bet');
  }
